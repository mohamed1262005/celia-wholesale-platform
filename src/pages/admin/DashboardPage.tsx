...
export function DashboardPage() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 pb-28 max-w-7xl mx-auto px-3 sm:px-6 w-full overflow-x-hidden">
      ...
      {/* 2. الكروت الأربعة الرئيسية (KPIs) - متجاوبة 1 عمود على الموبايل و 2 أو 4 للشاشات الكبيرة */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div className="rounded-2xl border border-pink-200 shadow-xs p-4 sm:p-5 flex items-center justify-between bg-gradient-to-br from-pink-50/70 to-white">
          <div className="min-w-0 flex-1">
            ...
            <span className="text-[11px] sm:text-xs text-emerald-600 font-semibold mt-1 inline-block">25% عن الشهر الماضي</span>
          </div>
          <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-2xl bg-[#FF6FA5] text-white flex items-center justify-center shadow-md shadow-pink-500/20 flex-shrink-0 ms-3">
        ...
        <div className="rounded-2xl border border-teal-200 shadow-xs p-4 sm:p-5 flex items-center justify-between bg-gradient-to-br from-teal-50/70 to-white">
          <div className="min-w-0 flex-1">
            ...
            <span className="text-[11px] sm:text-xs text-emerald-600 font-semibold mt-1 inline-block">18% عن الشهر الماضي</span>
          </div>
          <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-2xl bg-[#00C7B7] text-white flex items-center justify-center shadow-md shadow-teal-500/20 flex-shrink-0 ms-3">
        ...
        <div className="rounded-2xl border border-amber-200 shadow-xs p-4 sm:p-5 flex items-center justify-between bg-gradient-to-br from-amber-50/70 to-white">
          <div className="min-w-0 flex-1">
            ...
            <span className="text-[11px] sm:text-xs text-emerald-600 font-semibold mt-1 inline-block">30% عن الشهر الماضي</span>
          </div>
          <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-2xl bg-amber-500 text-white flex items-center justify-center shadow-md shadow-amber-500/20 flex-shrink-0 ms-3">
        ...
        <div className="rounded-2xl border border-purple-200 shadow-xs p-4 sm:p-5 flex items-center justify-between bg-gradient-to-br from-purple-50/70 to-white">
          <div className="min-w-0 flex-1">
            ...
            <span className="text-[11px] sm:text-xs text-purple-600 font-semibold mt-1 inline-block">متوفر متاح</span>
          </div>
          <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-2xl bg-purple-600 text-white flex items-center justify-center shadow-md shadow-purple-600/20 flex-shrink-0 ms-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-xs p-4 sm:p-6 overflow-hidden">
          <h2 className="font-bold text-gray-900 text-sm flex items-center gap-2 mb-4">
            ...
            <span className="truncate">{lang === 'ar' ? 'مبيعات خلال آخر أيام' : 'Sales — last 7 days'}</span>
          </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-xs p-4 sm:p-6 overflow-hidden">
          <div className="flex items-center justify-between mb-4 gap-2">
            <h2 className="font-bold text-gray-900 text-sm truncate">{t('recentOrders')}</h2>
                  ...
                  {recentOrders.map(order => (
                    <tr key={order.id} className="hover:bg-gray-50/60 transition-colors">
                      <td className="py-3.5 px-3 text-gray-900 font-mono font-bold">
                        <Link to={`/admin/orders/${order.id}`} className="hover:text-primary-600 truncate block max-w-[100px]">
                          {order.order_number}
                        </Link>
                      </td>
                      <td className="py-3.5 px-3 text-gray-700 font-medium truncate max-w-[120px]">{order.customer_name}</td>
                      <td className="py-3.5 px-3 text-gray-400 whitespace-nowrap">{formatDate(order.created_at, lang)}</td>
                      <td className="py-3.5 px-3 text-gray-900 font-mono font-bold whitespace-nowrap">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-xs p-4 sm:p-6 overflow-hidden">
          <h2 className="font-bold text-gray-900 text-sm mb-4 flex items-center gap-2 truncate">
            <TrendingUp className="w-4 h-4 text-primary-500 flex-shrink-0" />
            <span className="truncate">{t('topProducts')}</span>
          </h2>
            ...
            <div className="space-y-3">
              {topProducts.map((product, idx) => (
                <div key={idx} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-gray-50 transition-colors">
                  <span className="text-xs text-gray-300 w-4 font-bold flex-shrink-0 text-center">{idx + 1}</span>
